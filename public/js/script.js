...
(function( $ ) {
	...
	$.fn.boSlider = function( options ) {
		...
		if (options.slideShow == true) {
			timeoutslot = setTimeout(slider.autoPlay.bind(slider), options.interval);
		}		
		return $(".bo-slider");
	};
	...
	function initSlider(options, element) {
		...
		$(element[0]).children().each(function() {
			...
			obj['data-interval'] = li.attr("data-interval");
			obj['data-id'] = li.attr("data-id");
		...
		$(element[0]).replaceWith(function () {
			...
			for (var i = 0; i < slides.length; i++) {
				...
				if (slides[i]['data-type'] == "image") { // Template for images
					elements += "<div data-id='"+ slides[i]['data-id'] + "' data-interval='"+ slides[i]['data-interval'] + "' data-type='"+ slides[i]['data-type'] + "' class='bo-slide " + options.animation + " " + active +"'>\n";
					elements += "<img src='"+ slides[i]['data-url'] + "' alt='" + slides[i]['data-type'] +"'>\n";
					elements += "</div>\n";
				} else if (slides[i]['data-type'] == "video") { // Template for videos
					elements += "<div data-id='"+ slides[i]['data-id'] + "' data-interval='"+ slides[i]['data-interval'] + "' data-type='"+ slides[i]['data-type'] + "' class='bo-slide "+ options.animation + " " + active +"'>\n";
					elements += "<video width='100%' height='500px' autoplay muted loop>\n";
	...
	function Slider(options, slides) {
		...
		this.showSlide = function () {
			...
			this.options.interval = $(this.slides[this.active]['ref']).data('interval');
			...
			if($(this.slides[this.active]['ref']).data('type') == 'video'){
				$(this.slides[this.active]['ref']).find("video").prop('muted', false);
			}
		}
		...
		this.autoPlay = function () {
			if (this.options.slideShow == true && this.playing == false) {				
				...
				timeoutslot = setTimeout(this.autoPlay.bind(this), this.options.interval);
			}
		}
